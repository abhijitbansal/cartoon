...
//!
//! Reuses the same allowlist as the hook (`hook::ALWAYS` etc.), so the two
//! integrations wrap exactly the same commands.
//!
...
pub fn script() -> String {
    ...
    s.push_str("# cartoon shim — transparently wrap noisy dev tools.\n");
    s.push_str("# Generated by `cartoon shim`. Disable per-shell with CARTOON_NO_SHIM=1.\n");
    // Shared dispatcher: run the tool under cartoon, or bare when disabled or
    // cartoon is missing. `command` bypasses these functions, so the child
    // never re-enters the shim (cartoon also execs the binary directly).
    s.push_str("__cartoon_run() {\n");
    s.push_str(
