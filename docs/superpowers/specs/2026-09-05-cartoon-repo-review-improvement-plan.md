...
- **CI is disabled, deliberately.** The `ci` workflow shows
  `disabled_manually` on GitHub since 2026-06-15; PRs #5 through #10 merged
  with no CI. Decision (maintainer, 2026-09-05): it stays disabled because
  GitHub Actions runner minutes cost money the maintainer does not want to
  pay. The local gate replaces it: `cargo fmt --check`,
  `cargo clippy --all-targets -- -D warnings`, `cargo test` before every
  commit, and the PR checklist records that they ran.
...
1. Record the CI-disabled decision in `ci.yml`, `CONTRIBUTING.md`, `RELEASING.md` (CI stays off: runner minutes cost).
...
All six phases land on `feat/wrap-scripts-project-config` as one commit per
phase and one PR (maintainer decision, 2026-09-05); implementation plan:
`docs/superpowers/plans/2026-09-05-cartoon-review-fixes-all-phases.md`.
